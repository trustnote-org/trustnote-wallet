...
function initWallet() {
	...
	function setWalletNameAndColor(walletName) {
		...
		var color = root.config.colorFor ? root.config.colorFor[root.focusedClient.credentials.walletId] : '#0095ff';
		if(!color) color = '#0095ff';
	...
	function setWalletsInMenu() {
		...
		for (var key in root.walletClients) {
			...
			html += '<li onclick="wallet.selectWallet(\'' + walletId + '\')" id="w' + walletId + '" class="nav-item ' + (walletId === selectedWalletId ? 'selected' : '') + '">' +
				'<a class="oh"><div class="avatar-wallet " style="background-color: ' + (colors && colors[walletId] ? colors[walletId] : '#0095ff') + '">' + credentials.walletName.substr(0, 1) + ' </div>' +
