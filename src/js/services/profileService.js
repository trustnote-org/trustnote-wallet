...
angular.module('trustnoteApp.services').factory('profileService', function profileServiceFactory($rootScope, $location, $timeout, $filter, $log, lodash, storageService, bwcService, configService, isCordova, gettext, gettextCatalog, nodeWebkit, uxLanguage) {
    ...
    function unlockWalletAndInitDevice() {
        ...
        var removeListener = $rootScope.$on('Local/BalanceUpdated', function () {
            ...
            root.insistUnlockFC(null, false, function () {
    ...
    // continue to request password until the correct password is entered or click cancel button
    root.insistUnlockFC = function (error_message, allowCancel, cb) {
        root.unlockFC(error_message, function (err) {
            ...
            if (!allowCancel) {
                $timeout(function () {
                    root.insistUnlockFC(err.message, allowCancel, cb);
                }, 100);
            }
            else {
                if (root.clickCancel) {
